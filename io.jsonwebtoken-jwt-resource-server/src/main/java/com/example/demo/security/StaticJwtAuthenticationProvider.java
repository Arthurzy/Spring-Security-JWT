    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (kid == null) {
            ...
            try {
                ...
                byte[] apiKeySecretBytes = "asdfadf".getBytes(StandardCharsets.UTF_8);
                Key signingKey = new SecretKeySpec(apiKeySecretBytes, signatureAlgorithm.getJcaName());
                claims = Jwts.parser()
                        .setSigningKey(signingKey)
                        .parseClaimsJws(bearer.getToken()).getBody();
//                jwt = JWTParser.parse(bearer.getToken());
//                jwt = jwtDecoder().decode(bearer.getToken());
            ...
            AbstractAuthenticationToken token = this.converter.convertclaim(claims);
